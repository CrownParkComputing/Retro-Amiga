...
| JIT bug | `src/jit/arm/compemu_support_arm.cpp` | Block compilation, flags, and register/flag state handling |
...
- **WinUAE upstream**: Core files (`custom.cpp`, `newcpu.cpp`, `memory.cpp`, etc.) track WinUAE — keep local divergences narrow and merge-friendly
- **JIT debugging**: Start with `src/jit/arm/compemu_support_arm.cpp`, `src/jit/arm/compemu_arm.h`, and `src/jit/x86/compemu_support_x86.cpp`
- **Troubleshooting**: Follow a normal edit-build-run-test loop and validate behavior on the target platform
