...
#include "gui.h"
#include "debug.h"
#include "uae4arm/upstream_overrides.h"
#ifdef WITH_SNDBOARD
#include "sndboard.h"
#endif
#ifdef AVIOUTPUT
...
static bool audio_state_channel2 (int nr, bool perfin)
{
	...
	if ((cdp->state & 15) == 2 || (cdp->state & 15) == 3) {
		...
		if (cdp->dmaofftime_active && !old_dma && chan_ena) {
			...
			if (usehacks() && uae4arm_allows_audio_dma_wait_hack(currprefs.cachesize, currprefs.m68k_speed, regs.instruction_cnt, cdp->dmaofftime_cpu_cnt)) {
				if (warned >= 0) {
					warned--;
