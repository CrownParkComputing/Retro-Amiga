...
{
    // Check if address is in ROM - redirect to special handler to prevent crashes
    // This fixes game loading crashes when JIT attempts writes to Kickstart ROM
    if (isinrom(address)) {
        switch (size) {
        case 1: jnf_MEM_WRITEMEMBANK(address, source, SIZEOF_VOID_P * 5); break;
        case 2: jnf_MEM_WRITEMEMBANK(address, source, SIZEOF_VOID_P * 5); break;
        case 4: jnf_MEM_WRITEMEMBANK(address, source, SIZEOF_VOID_P * 5); break;
        }
        return;
    }
