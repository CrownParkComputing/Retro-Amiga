...
static int target_get_display_scanline2(int displayindex)
{
	...
	if (!renderer)
		return -1;
	float diff = static_cast<float>(read_processor_time() - renderer->vsync_state().wait_vblank_timestamp);
	if (diff < 0)
		return -1;
...
int target_get_display_scanline(const int displayindex)
{
	...
	if (!renderer)
		return -1;
	if (!renderer->vsync_state().scanlinecalibrating && calculated_scanline) {
...
static void display_vblank_thread(struct AmigaMonitor* mon)
{
	...
	IRenderer* renderer = mon ? get_renderer(mon->monitor_id) : get_renderer();
	if (!renderer)
		return;

	if (renderer->vsync_state().waitvblankthread_mode > 0)
		return;
...
{
	if (!mon) {
		write_log(_T("display_param_init: null monitor\n"));
		return;
	}
	IRenderer* renderer = get_renderer(mon->monitor_id);
	if (!renderer) {
		write_log(_T("display_param_init: renderer unavailable for monitor %d\n"), mon->monitor_id);
		return;
	}
	const struct amigadisplay* ad = &adisplays[mon->monitor_id];
	struct apmode* ap = ad->picasso_on ? &currprefs.gfx_apmode[APMODE_RTG] : &currprefs.gfx_apmode[APMODE_NATIVE];
	...
	renderer->vsync_state().wait_vblank_display = getdisplay(&currprefs, mon->monitor_id);
	if (renderer->vsync_state().wait_vblank_display) {
		renderer->vsync_state().wait_vblank_display->HasAdapterData = true; // SDL displays always have adapter data
	}
	if (!renderer->vsync_state().wait_vblank_display || !renderer->vsync_state().wait_vblank_display->HasAdapterData) {
		write_log(_T("Selected display mode does not have adapter data!\n"));
	}
	renderer->vsync_state().scanlinecalibrating = true;
	target_calibrate_spin();
	renderer->vsync_state().scanlinecalibrating = false;
...
void sortdisplays()
{
	...
	while (md->monitorname) {
		...
		// Android (and some embedded platforms) report 0 fullscreen modes.
		// Fall back to the desktop display mode so RTG/Picasso96 has at
		// least one resolution to register with the guest.
		if (num_disp_modes == 0 || !modes) {
