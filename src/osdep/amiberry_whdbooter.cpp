...
#ifdef __ANDROID__
	const bool preserve_onscreen_joystick = prefs->onscreen_joystick;
	const bool preserve_vkbd_enabled = prefs->vkbd_enabled;
	const bool preserve_input_default_osk = prefs->input_default_onscreen_keyboard;
#endif
...
#ifdef __ANDROID__
		prefs->onscreen_joystick = preserve_onscreen_joystick;
		prefs->vkbd_enabled = preserve_vkbd_enabled;
		prefs->input_default_onscreen_keyboard = preserve_input_default_osk;
#endif
		return;
	}
...
#ifdef __ANDROID__
	// The Android UI can write explicit touch-control preferences before
...
#ifdef __ANDROID__
	// Android manages its own gameplay keyboard button and should not re-enable
	// the native Amiberry on-screen joystick/keyboard during WHDLoad autoload.
	prefs->onscreen_joystick = preserve_onscreen_joystick;
	prefs->vkbd_enabled = preserve_vkbd_enabled;
	prefs->input_default_onscreen_keyboard = preserve_input_default_osk;
#endif
