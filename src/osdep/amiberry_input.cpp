...
static int cd32pad_device_index = -1;
constexpr auto analog_upper_bound = 32767;

// Android custom controller button remap table.
// button_remap[physical_sdl_button] = remapped_sdl_button.
// Identity by default; modified by apply_android_controller_remap().
static int button_remap[SDL_GAMEPAD_BUTTON_COUNT];
static bool button_remap_active = false;

// CD32 target index → default SDL button that the WinUAE input system expects
static const int cd32_target_to_sdl[7] = {
	SDL_GAMEPAD_BUTTON_SOUTH,          // 0 = Red
	SDL_GAMEPAD_BUTTON_EAST,           // 1 = Blue
	SDL_GAMEPAD_BUTTON_WEST,           // 2 = Green
	SDL_GAMEPAD_BUTTON_NORTH,          // 3 = Yellow
	SDL_GAMEPAD_BUTTON_START,          // 4 = Play
	SDL_GAMEPAD_BUTTON_LEFT_SHOULDER,  // 5 = RWD (L)
	SDL_GAMEPAD_BUTTON_RIGHT_SHOULDER, // 6 = FFW (R)
};

void apply_android_controller_remap(const int* sdl_to_target, int count)
{
	// Reset to identity
	for (int i = 0; i < SDL_GAMEPAD_BUTTON_COUNT; i++)
		button_remap[i] = i;

	// Build the remap: for each physical SDL button that has a CD32 target,
	// remap it to the SDL button that the default CD32 mapping expects
	bool any = false;
	for (int sdl_btn = 0; sdl_btn < count && sdl_btn < SDL_GAMEPAD_BUTTON_COUNT; sdl_btn++)
	{
		int target = sdl_to_target[sdl_btn];
		if (target >= 0 && target < 7)
		{
			button_remap[sdl_btn] = cd32_target_to_sdl[target];
			any = true;
		}
	}
...
int get_onscreen_cd32pad_device_index()
{
	return cd32pad_device_index;
}

void ensure_onscreen_cd32pad_registered()
{
	if (cd32pad_device_index >= 0)
		return;
	if (num_joystick >= MAX_INPUT_DEVICES)
		return;

	cd32pad_device_index = num_joystick;
	struct didata* did = &di_joystick[cd32pad_device_index];
	cleardid(did);
	did->name = "On-Screen CD32 Pad";
	did->joystick_name = "On-Screen CD32 Pad";
	did->is_controller = false;
	did->controller = nullptr;
	did->joystick = nullptr;
	did->joystick_id = -1;
	...
	static const char* btn_names[] = {
		"Red (Fire)", "Blue (2nd Fire)", "Green", "Yellow",
		"Play", "Rewind", "Fast Forward"
	};
	for (int i = 0; i < 7; i++) {
		did->buttonmappings[i] = i;
		did->buttonname[i] = btn_names[i];
		did->buttonsort[i] = i;
	}
	fixthings(did);
	num_joystick++;
	write_log("On-Screen CD32 Pad registered as JOY%d\n", cd32pad_device_index);
}
...
void read_controller_button(const int id, const int button, const int state)
{
	...
	// Apply custom button remap if active (from Android controller mapping)
	const int mapped_button = (button_remap_active && button >= 0 && button < SDL_GAMEPAD_BUTTON_COUNT)
		? button_remap[button] : button;
	...
	if (isfocus() || currprefs.inactive_input & 4)
	{
		...
		setjoybuttonstate(id, mapped_button + held_offset, state);
	}
}
